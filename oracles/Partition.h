...
public:
    ...
    std::vector<T> ground_set;                  //a vector of elements of type T. each element t \in T is represented internally in the oracle by the index of t in the ground_set
    ...
    std::vector<int> category;                  //this is a vector where category[i]:=j means that the item t having index i in ground_set is in category j     
