...
using std::pair, std::vector, std::cout, std::endl, std::cerr, std::string, std::max, std::min;
...
pair<vector<vector<int>>, vector<Edge>> GenerateBlocksBipartiteGraph(int V, int n1) {
	...
	vector<Edge> edges;
	const int 	 n2 	 = V/n1;
	const int 	 degree	 = min(6,n2*3); //average degree
	const double p 		 = (double)degree/(double)V;
...
void assertGraphic(vector<Edge> forest, string name) {
	cerr << ">>assertGraphic: Asserting solution given by " << name << endl;
